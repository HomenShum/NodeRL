...
|---|---|---|---|---|
| `nodetrace` | ~80% | `src/nodeagent/capture/{types,pipeline,reasoning,guards,index}.ts`, `capture/substrate/{index,firecrawl,browserbase}.ts` | per-step `reward` + `cost {tokensIn,tokensOut,latencyMs}`, `episode_id`/`step_index`, `truncated`/`resumeFrom`, **JSONL export** | S |
