...
| `packages/nodetrace/src/index.ts` re-exports both halves, so importing the package root loads the capture half and needs `zod` + the AI SDK — even for a consumer who only wants the pure loop | A deliberate trade: one discoverable entry point per package was the goal, and subpath exports were the indirection this pass removed. Importing the module directly still loads nothing external. Verified by deleting `zod` from `node_modules`: `npm run demo` exits 0, `test/entrypoints.test.ts` fails, the other 13 test files pass. | `docs/codebase/STACK.md`, `packages/nodetrace/README.md` |
